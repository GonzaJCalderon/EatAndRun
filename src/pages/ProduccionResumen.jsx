    ...
    pedidos.forEach((p) => {
      Object.entries(p.pedido || {}).forEach(([dia, platos]) => {
        ...
        Object.entries(platos).forEach(([platoKey, datos]) => {
          ...
          resumenTemp[dia][nombre] = (resumenTemp[dia][nombre] || 0) + datos.cantidad;
          totalTemp[nombre] = (totalTemp[nombre] || 0) + datos.cantidad;
        });

        if (p.observaciones) {
          if (!obsTemp[dia]) obsTemp[dia] = [];
          obsTemp[dia].push(`• ${p.usuario?.nombre || "Usuario"}: ${p.observaciones}`);
        }
      });
    });
